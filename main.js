...
const starts = async (abilbotz = new WAConnection()) => {
    ...
    abilbotz.on('open', () => {
        success('2', 'TERHUBUNG JANGAN LUPA SUBSCRIBE SANZY YT')
    })
...
abilbotz.on("group-participants-update", async (anu) => {
    ...
    try {
      ...
      if (anu.action == "add" && mem.includes(abilbotz.user.jid)) {
        abilbotz.sendMessage(anu.jid, "Halo!.. saya SANZY BOTZ saya akan membatu mempermudah kehidupan..seperti membuat sticker dan lain-lain. untuk memulai silahkan ketik !menu.", "conversation")
      }
      if (!isWelkom) return
      if (anu.action == "add" && !mem.includes(abilbotz.user.jid)) {
        ...
        sendButImage(mdata.id, wel, "©Created : Sanzy Botz", buff, but)
      }
      if (!isWelkom) return
      if (anu.action == "remove" && !mem.includes(abilbotz.user.jid)) {
        ...
        sendButImage(mdata.id, out, "©Created : Sanzy Botz", buff, but)
      }
